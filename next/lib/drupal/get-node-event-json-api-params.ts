...
export type ResourceType = "node--event" | "node--event" | "node--article";

export function getNodeEventJsonApiParams(resourceType: ResourceType) {
  const apiParams = new DrupalJsonApiParams().addFilter(
    "field_site.meta.drupal_internal__target_id",
    env.DRUPAL_SITE_ID,
  );
  // The event content type has paragraphs, stored in the "field_content_elements" field:
  ...
  // The frontevent content type has paragraphs, stored in the "field_content_elements" field:
  if (resourceType === "node--event") {
    apiParams
      .addInclude([
        "field_content_elements",
